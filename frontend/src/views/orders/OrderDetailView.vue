    ...
    <div v-if="order" class="space-y-6">
      ...
      <div class="card p-6">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between border-b border-neutral-100 pb-4 mb-4">
          <div>
            ...
            <div class="mt-1 font-mono text-sm text-neutral-500 bg-neutral-100 inline-block px-2 py-1 rounded">{{ order.tracking_token }}</div>
          </div>
          <div class="text-lg font-black text-primary">{{ Number(order.total_amount).toLocaleString('fa-IR') }} تومان</div>
        </div>
        
        <div class="grid gap-4 text-sm md:grid-cols-2">
          <div><span class="text-neutral-400 block text-xs mb-1">مشتری</span> <span class="font-bold">{{ order.customer_name_snapshot || 'بدون نام' }}</span></div>
          <div><span class="text-neutral-400 block text-xs mb-1">شهر</span> <span class="font-bold">{{ order.customer_city || 'نامشخص' }}</span></div>
          <div><span class="text-neutral-400 block text-xs mb-1">جنسیت</span> <span class="font-bold">{{ translateEnum(genderMap, order.customer_gender) }}</span></div>
          <div><span class="text-neutral-400 block text-xs mb-1">منبع سفارش</span> <span class="font-bold">{{ translateEnum(sourceMap, order.order_source) }}</span></div>
          <div><span class="text-neutral-400 block text-xs mb-1">تعداد</span> <span class="font-bold">{{ order.quantity }} عدد</span></div>
          <div class="md:col-span-2"><span class="text-neutral-400 block text-xs mb-1">یادداشت</span> <span class="font-bold">{{ order.notes || '-' }}</span></div>
        </div>
      </div>
      ...
      <div class="card p-6">
        ...
        <form class="grid gap-5 md:grid-cols-2" @submit.prevent="updateOrder">
          ...
          <button class="btn-primary md:col-span-2 py-3 mt-2 shadow-lg shadow-primary/20" :disabled="isLoading">
      ...
      <div class="card p-6">
        <h2 class="text-lg font-bold mb-6">تاریخچه وضعیت</h2>
        <div class="space-y-4">
          <div v-for="log in logs" :key="log.created_at" class="flex gap-4 p-4 rounded-2xl bg-neutral-50 border border-neutral-100 relative hover:bg-fuchsia-50 transition-colors">
            <div class="absolute right-0 top-1/2 -translate-y-1/2 w-1.5 h-full rounded-r-2xl bg-primary"></div>
            <div>
              <div class="font-bold text-neutral-900">{{ translateEnum(orderStatusMap, log.status) }}</div>
              <div class="mt-1 text-sm text-neutral-600">{{ log.note }}</div>
              <div class="mt-2 text-xs text-neutral-400 font-mono" dir="ltr">{{ new Date(log.created_at).toLocaleString('fa-IR') }}</div>
            </div>
          </div>
        </div>
      </div>
