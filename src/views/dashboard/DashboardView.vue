  ...
  <AppShell>
    <div class="space-y-6">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <div class="card p-5">
          <h2 class="font-bold">تنظیمات قابل مشاهده برای خریدار</h2>
          <label class="mt-4 flex items-center justify-between rounded-2xl border border-neutral-200 p-4 text-sm">
            <span>نمایش منبع سفارش در لینک پیگیری</span>
            <input v-model="exposeOrderSource" type="checkbox" class="h-5 w-5" />
          </label>
        ...
        <div class="card p-5">
          ...
          <div class="mt-4 space-y-2 text-sm text-neutral-600">
            ...
            <div class="flex justify-between"><span>تعداد محصول</span><span>{{ shop.product_limit || 0 }}</span></div>
            <div class="flex justify-between"><span>تعداد همکار</span><span>{{ shop.operator_limit || 0 }}</span></div>
            <div class="flex justify-between"><span>وضعیت</span><span>{{ shop.status || '-' }}</span></div>
          </div>
        </div>
      </div>
    </div>
